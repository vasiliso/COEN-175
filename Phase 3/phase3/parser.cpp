...
static void declarator(int typespec)
{
    ...
    if (lookahead == '[') {
		match('[');
		int length = stoi(lexbuf);
		match(NUM);
		match(']');
		var1 = Type(typespec, ptr, length);
    }
	else{
		var1 = Type(typespec, ptr);
	}
	declareVariable(id, var1);
}
...
static void globalDeclarator(int typespec)
{
    ...
    if (lookahead == '(') {
		...
		declareFunction(id, Type(typespec, ptr, params));
    } else if (lookahead == '[') {
		match('[');
		int length = stoi(lexbuf);
		match(NUM);
		match(']');
		declareVariable(id, Type(typespec, ptr, length));
    }else{
		declareVariable(id, Type(typespec, ptr));
	}
}
...
static void globalOrFunction()
{
    ...
    if (lookahead == '[') {
		...
		if (lookahead == ')') {
			...
			Parameters *params;
			params = NULL;
			var1 = Type(typespec, ptr, params);
			declareFunction(id, var1);
			remainingDeclarators(typespec);
		} else {
			Parameters * params = new Parameters;
			var1 = Type(typespec, ptr, params);
			defineFunction(id, var1);
			openScope();
			parameters(params);
			match(')');
			match('{');
